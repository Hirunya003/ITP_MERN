...
import { FiEdit, FiTrash2, FiPlus, FiDownload, FiPieChart, FiBarChart2, FiUsers, FiFileText } from 'react-icons/fi';
...
const AdminDashboard = () => {
  ...
  // Only allow useradmin@example.com to access this page
  if (!user || user.email !== 'useradmin@example.com') {
    return <Navigate to="/" />;
  }
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // For this demo, we'll generate sample data based on existing users
        ...
        switch (reportType) {
          case 'userRoles':
            ...
            data = {
              ...
              type: 'pie',
              description: 'Shows the distribution of users by their assigned roles in the system.'
            };
            break;
          ...
          case 'userRegistration':
            ...
            const monthlyRegistration = filteredUsers.reduce((months, user) => {
              ...
              // Format as "January 2023" instead of "1/2023"
            ...
            const sortedMonths = Object.keys(monthlyRegistration).sort((a, b) => {
            ...
            data = {
              ...
              type: 'bar',
              description: 'Shows the number of new user registrations per month.',
          ...
          case 'userActivity':
            // Demo data for user activity (would be fetched from backend in a real app)
            data = {
              ...
              rows: users.slice(0, 5).map(user => [
                ...
                new Date(Date.now() - Math.floor(Math.random() * 7 * 24 * 60 * 60 * 1000)).toLocaleDateString() // Random recent date
              ]),
              type: 'table',
              description: 'Shows the most active users based on login frequency.'
            };
            break;
  ...
  const exportReport = () => {
    ...
    enqueueSnackbar('Report exported successfully as CSV', { variant: 'success' });
  };
  ...
  const exportReportAsPDF = () => {
    ...
    try {
      ...
      if (reportData.filteredCount !== undefined) {
        doc.text(`Showing data for ${reportData.filteredCount} out of ${reportData.totalCount} users`, 14, 46);
      }
      ...
      if (reportData.type === 'table') {
        ...
        const chartData = reportData.labels.map((label, index) => [label, reportData.values[index]]);
        autoTable(doc, {
          head: [['Category', 'Value']],
          body: chartData,
          startY: finalY,
        ...
        finalY = doc.lastAutoTable ? doc.lastAutoTable.finalY : finalY + 20;
        doc.text("Note: For better visualization, view the report on screen.", 14, finalY + 10);
      }
      ...
      for(let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        doc.setFontSize(10);
        doc.text(`Page ${i} of ${pageCount} - SuperMart Admin Dashboard`, 14, doc.internal.pageSize.height - 10);
      }
      
      doc.save(`${reportData.title.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`);
      enqueueSnackbar('Report exported successfully as PDF', { variant: 'success' });
    } catch (error) {
      ...
      return (
        <div className="mt-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <h4 className="text-lg font-semibold text-center mb-4">{reportData.title}</h4>
                ...
                {reportData.labels.map((label, index) => (
                  <div key={index} className="flex items-center p-2">
                    <div 
                      ...
                      style={{ backgroundColor: `hsl(${index * (360 / reportData.labels.length)}, 70%, 60%)` }}
                    ></div>
                    <span className="text-sm">{label}: {reportData.values[index]}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <p className="text-sm text-gray-500 mt-4">{reportData.description}</p>
          </div>
        </div>
      );
    }
            ...
            <>
              ...
              {activeTab === 'reports' && (
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg mb-6 border">
                    <h4 className="text-lg font-medium mb-4">Generate Report</h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div>
                        ...
                        >
                          <option value="userRoles">User Roles Distribution</option>
                          <option value="userRegistration">User Registration by Month</option>
                          <option value="userActivity">User Activity Report</option>
                        </select>
                      </div>
                      
                      {reportType === 'userRegistration' && (
                        <div>
                  ...
                  {reportData && !generatingReport && (
                    <div className="bg-white border rounded-lg p-4">
                      <div className="flex justify-between items-center mb-4">
                        <h4 className="text-lg font-medium text-center">{reportData.title}</h4>
